    ...
    if tokenSet[count.value][lexi.CLASS_PART] == "IMPORT":
        ...
        if tokenSet[count.value][lexi.CLASS_PART] == "IDENTIFIER":
            ...
            if tokenSet[count.value][lexi.CLASS_PART] == "END_OF_STATEMENT":
                ...
                if import_(count):
                    return True
    elif tokenSet[count.value][lexi.CLASS_PART] == "CLASS" or "$":
        return True
...
def inheritanceBodyRecursive(count):
    if tokenSet[count.value][lexi.CLASS_PART] ==  lexi.ARGS_SEPERATOR_OP:
        count+=1
        if tokenSet[count.value][lexi.CLASS_PART] == "IDENTIFIER":
            count+=1
            if inheritanceBodyRecursive(count):
...
def init(count):
    if tokenSet[count.value][lexi.CLASS_PART] == "AS_OP":
        ...
        if condition(count):
            return True
    elif tokenSet[count.value][lexi.CLASS_PART] ==  lexi.ARGS_SEPERATOR_OP or "END_OF_STATMENT":
        return True
    return False
...
def objectInit(count):
    if tokenSet[count.value][lexi.CLASS_PART] == "AS_OP":
        ...
        if lfObjectInit(count):
            return True
    elif tokenSet[count.value][lexi.CLASS_PART] ==  lexi.ARGS_SEPERATOR_OP or "END_OF_STATEMENT":
        return True
    return False
...
def condition_dash(count):
    if tokenSet[count.value][lexi.CLASS_PART] == "OR":
        ...
        if ae(count):
            if condition_dash(count):
                return True
    elif tokenSet[count.value][lexi.CLASS_PART] ==  lexi.ARGS_SEPERATOR_OP or "END_OF_STATEMENT" or "ROUND_BRACKET_CLOSE" or "IDENTIFIER" or "CURLY_BRACKET_CLOSE" or "DATA_TYPE" or "WHILE" or "IF" or "FOR" or "FOR_EACH" or "SEND":
        return True
    return False
...
def t_Dash(count):
    if mdm(count):
        if f(count):
            if t_Dash(count):
                return True
    elif tokenSet[count.value][lexi.CLASS_PART] == "PM" or  lexi.ARGS_SEPERATOR_OP or "END_OF_STATEMENT" or "ROUND_BRACKET_CLOSE" or "IDENTIFIER" or "CURLY_BRACKET_CLOSE" or "DATA_TYPE" or "WHILE" or "IF" or "FOR" or "FOR_EACH" or "SEND":
        return True
    return False
...
def argumentOrNull(count):
    if argumentBody(count):
        ...
        return True
    elif tokenSet[count.value][lexi.CLASS_PART] == "METHOD_OP" or "END_OF_STATEMENT" or lexi.ARGS_SEPERATOR_OP or "ROUND_BRACKET_CLOSE" or "IDENTIFIER" or "CURLY_BRACKET_CLOSE" or "DATA_TYPE" or "WHILE" or "IF" or "FOR" or "FOR_EACH" or "SEND":
        return True
    return False
    ...
    if tokenSet[count.value][lexi.CLASS_PART] == "METHOD_OP":
        ...
        if tokenSet[count.value][lexi.CLASS_PART] == "IDENTIFIER":
            ...
            if  argumentOrNull(count):
                if objFunctionArrayInvocationRecursive(count):
                    return True
    elif tokenSet[count.value][lexi.CLASS_PART] == "END_OF_STATEMENT" or lexi.ARGS_SEPERATOR_OP or "ROUND_BRACKET_CLOSE" or "IDENTIFIER" or "CURLY_BRACKET_CLOSE" or "DATA_TYPE" or "WHILE" or "IF" or "FOR" or "FOR_EACH" or "SEND":
        return True
    return False
        ...
        if arrayElements(count):
            if tokenSet[count.value][lexi.CLASS_PART] == "CURLY_BRACKET_CLOSE":
                ...
                if arraySeparator2d(count):
                    return True
    elif tokenSet[count.value][lexi.CLASS_PART] == "CURLY_BRACKET_CLOSE" or  lexi.ARGS_SEPERATOR_OP:
        return True
    return False
...
def sstAllFunctions(count):
    if argumentBody(count):
        ...
        if objMethodAttributeRecursive(count):
            if tokenSet[count.value][lexi.CLASS_PART] == "END_OF_STATEMENT":
                count+=1
                return True
    return False
